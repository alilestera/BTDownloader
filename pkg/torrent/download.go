	...
	for state.downloaded < task.length {
		if !conn.Choked {
			for state.backlog < MAXBACKLOG && state.requested < task.length {
				length := BLOCKSIZE
				if length > task.length-state.requested {
...
func (t *TorrentTask) peerRoutine(peer PeerInfo, taskQueue chan *pieceTask, resultQueue chan *pieceResult) {
	...
	for task := range taskQueue {
		...
		if err != nil {
			taskQueue <- task
			fmt.Println("fail to download piece, error got " + err.Error())
			return
		}
...
func Download(task *TorrentTask) error {
	...
	for count < len(task.PieceSHA) {
		...
		fmt.Printf("downloading, progress: (%.2f%%)\n", percent)
	}
	close(taskQueue)
	close(resultQueue)
	...
	if err != nil {
		fmt.Println("fail to write file: " + task.FileName)
		return err
	}
	return nil
}
